...
function PushInfoToZt(form)
{
	...
	window.open('special/PushToZt.php?classid=<?=$classid?>&id='+id,'PushToZt','width=360,height=500,scrollbars=yes,left=300,top=150,resizable=yes');
}
//修改信息
function editnews(id,classid,bclassid){
if(document.getElementById("newck").checked){
   art.dialog.open('AddNews.php?enews=EditNews&id='+id+'&classid='+classid+'&bclassid='+bclassid+'<?=$addecmscheck?>',
	...
	window.open('AddNews.php?enews=EditNews&id='+id+'&classid='+classid+'&bclassid='+bclassid+'<?=$addecmscheck?>','','fullscreen=1')
	}
}
//简改信息
function jeditnews(id,classid,bclassid){
art.dialog.open('info/EditInfoSimple.php?enews=EditNews&id='+id+'&classid='+classid+'&bclassid='+bclassid+'<?=$addecmscheck?>',
...
art.dialog.open('ecmsinfo.php?enews=DelNews&id='+id+'&classid='+classid+'&bclassid='+bclassid+'<?=$addecmscheck?>',
...
<div style=" background:none; float:right; padding-right:20px;">[<a href="AddClass.php?enews=EditClass&classid=<?=$classid?>">栏目设置</a>] 
        [<a href="AddInfoClass.php?enews=AddInfoClass&newsclassid=<?=$classid?>">增加采集</a>] 
        [<a href="ListInfoClass.php">管理采集</a>] [<a href="ecmschtml.php?enews=ReAllNewsJs&from=<?=$phpmyself?>">刷新所有信息JS</a>]</div>
</div>
<div id="tab" style="margin-top:35px;">
<div class="ui-tab-bd">
<div class="ui-tab-content">
<form name="AddNewsForm" method="get">
        	<h3><span>信息列表 <a href="ListNews.php?bclassid=<?=$bclassid?>&classid=<?=$classid?>" title="已发布信息总数：<?=$classinfos?>" <?=$indexchecked==1?' class="glon"':'class="gl"'?> style=" margin-left:10px;">已发布 (<?=$classinfos?>)</a>
            <a href="ListNews.php?bclassid=<?=$bclassid?>&classid=<?=$classid?>&ecmscheck=1" <?=$indexchecked==0?' class="glon"':'class="gl"'?> title="待审核信息总数：<?=$classckinfos?>">待审核 (<?=$classckinfos?>)</a>
            <a href="ListInfoDoc.php?bclassid=<?=$bclassid?>&classid=<?=$classid?><?=$addecmscheck?>" class="gl">归档</a></span></h3>
</form>
            <div class="line"></div>
           	<div class="saixuan">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            <td> <div align="left"  class="anniuqun"> 
                <input type=button name=button value="增加信息" onClick="self.location.href='AddNews.php?enews=AddNews&ecmsnfrom=1<?=$addecmscheck?>&bclassid=<?=$bclassid?>&classid=<?=$classid?>'" class="addnews">
               <select name="dore">
                ...
                <input type="button" name="Submit12" value="提交" onclick="self.location.href='ecmsinfo.php?enews=AddInfoToReHtml<?=$addecmscheck?>&classid=<?=$classid?>&dore='+document.searchinfo.dore.value;">
              </div></td>
          </tr>
        </table>
      </td>
...
<input type=hidden name=doing value=0>
  ...
  <div class="xwlistmenu">
  	<table>
<tr> 
            <td width="60%"><font color="#ffffff"><a href="ListNews.php?bclassid=<?=$bclassid?>&classid=<?=$classid?><?=$addecmscheck?>&sear=1&showspecial=8">我的信息</a> 
              | <a href="ListNews.php?bclassid=<?=$bclassid?>&classid=<?=$classid?><?=$addecmscheck?>&sear=1&showspecial=5">签发信息</a> 
              | <a href="ListNews.php?bclassid=<?=$bclassid?>&classid=<?=$classid?><?=$addecmscheck?>&sear=1&showspecial=7">投稿信息</a> 
              | <a href="ListNews.php?bclassid=<?=$bclassid?>&classid=<?=$classid?><?=$addecmscheck?>&showretitle=1&srt=1" title="查询重复标题，并保留一条信息">查询重复标题A</a> 
              | <a href="ListNews.php?bclassid=<?=$bclassid?>&classid=<?=$classid?><?=$addecmscheck?>&showretitle=1&srt=0" title="查询重复标题的信息(不保留信息)">查询重复标题B</a> 
              | <a href="ecmsinfo.php?bclassid=<?=$bclassid?>&classid=<?=$classid?><?=$addecmscheck?>&enews=SetAllCheckInfo" title="本栏目所有信息全设为审核状态" onclick="return confirm('确认要操作?');">审核栏目全部信息</a></font> 
            </td>
            <td width="40%"> <div align="right"><font color="#ffffff"><a href="openpage/AdminPage.php?leftfile=<?=urlencode('../file/FileNav.php')?>&mainfile=<?=urlencode('../file/ListFile.php?type=9&classid='.$classid)?>&title=<?=urlencode('管理附件')?>" target="_blank">管理附件</a> | <a href=ReHtml/ChangeData.php target=_blank>更新数据</a> | <a href=../../ target=_blank>预览首页</a> | <a href="<?=$classurl?>" target=_blank>预览栏目</a></font></div></td>
          </tr>
        </table></div></td>
    </tr>
    </table>
...
<table class="comm-table2" cellspacing="0" id="changecolor">
	<tbody>
		<tr>
			...
			<th>点击</th>
            <th>评论</th>
			<th>操作 <span style="font-size:11px; font-family:宋体;"> <input name="newbox" type="checkbox" id="newck" value="" checked="checked"/>本窗口打开</span></th>
		</tr>
    <?php
	while($r=$empire->fetch($sql))
	{
		...
		if(empty($indexchecked))
		{
			...
			$titleurl="ShowInfo.php?classid=$r[classid]&id=$r[id]".$addecmscheck;
		}
		else
		{
			$checked="";
		...
		if($r[isqf])
		{
			...
			$qf="<a href='#ecms' onclick=\"window.open('workflow/DoWfInfo.php?classid=$r[classid]&id=$r[id]".$addecmscheck."','','width=600,height=520,scrollbars=yes');\">".$qf."</a>";
		}
		//标题图片
		$showtitlepic="";
		if($r[titlepic])
    ...
    <tr bgcolor="#FFFFFF" onmouseout="this.style.backgroundColor='#ffffff'" onmouseover="this.style.backgroundColor='#C3EFFF'" id=news<?=$r[id]?>> 
      ...
      <? if($ecmscheck){echo "<a href='/e/ecmsshop/view/?id=".$r[id]."&classid=".$r[classid]."' style='color:red;' target='_blank'>【预览未审核信息】</a>";}?>
          <?=$st?>
          <?=$showtitlepic?>
      ...
      <td><div align="center"><a href="pl/ListPl.php?id=<?=$r[id]?>&classid=<?=$classid?>&bclassid=<?=$bclassid?><?=$addecmscheck?>" target=_blank title="管理评论"><u> 
          <?=$r[plnum]?>
          </u></a></div></td>
      <td height="25"> <div align="center"><a href="javascript:editnews(<?=$r[id]?>,<?=$classid?>,<?=$bclassid?>)">修改</a> | <a href="javascript:jeditnews(<?=$r[id]?>,<?=$classid?>,<?=$bclassid?>)">简改</a> | <a href="javascript:delnews(<?=$r[id]?>,<?=$classid?>,<?=$bclassid?>)" onClick="return confirm('确认要删除？');">删除</a>
        </div></td>
    </tr>
    <?
	}
