...
$userid   =getcvar('mluserid');
	$zhi=$empire->fetch1("select ti_duanxin from {$dbtbpre}enewsmember where userid=$userid"); 
	if($zhi[ti_duanxin]!=0){
		$empire->query("UPDATE {$dbtbpre}enewsmember SET ti_duanxin = 0 WHERE userid=$userid");
	}
...
<div class="hy_nav">
            	<ul>
            		<li><a href="/e/member/msg/" class="selected">我的消息</a></li>
           		  <li><a href="/e/member/msg/AddMsg/?enews=AddMsg">发送消息</a></li>
            	</ul>
</div>
<div class="hytable yh">
			...
			while($r=$empire->fetch($sql))
			{
				...
				{
					$from_username="<a href='/e/space/?userid=".$r[from_userid]."' target='_blank'>".$r[from_username]."</a>";
				}
				//系统信息
				if($r['issys'])
				{
