...
//别人的申请
$frie_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.groupid in(1,2) and yaoqing like '%$userid%' order by a.userid desc";
...
<script type="text/javascript">
		$(function() {
			$('.sixinWrap a').on('click', function(event) {
				$.ajax({
					url: '/e/extend/yaoqing/index.php',
					type: 'post',
					dataType: 'html'
					// data: {param1: 'value1'},
				})
		...
		<div class="bd">
				...
				<ul class="lh dengdai">
					<?
					$friend_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.userid IN ($guanwhe) and a.groupid in(1,2) and b.userid not in($yao) and b.userid not in($str) and b.userid not in($whe) order by a.userid desc";
					$list=$empire->query($friend_sql);
					while($r=$empire->fetch($list))
					{	   	
					?>
						...
						<div class="sixinWrap  p-price clearfix">
					                	<?php				
					                        	if ($getuserid!=$r[userid]){
													...
													$follow='<a href="javascript:void()" onclick="follow('.$r[userid].')" class="button yiyaoqingBtn blue small" id="follow'.$r[userid].'">邀请</a>';	
													}
				...
				<ul class="lh dengdai">
					<!--我的邀请-->
					<?
					$friend_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.userid IN ($yao) and a.groupid in(1,2) and b.userid not in($whe) order by a.userid desc";
					$list=$empire->query($friend_sql);
					while($r=$empire->fetch($list))
					{	   	
					?>
						...
						<div class="sixinWrap  p-price clearfix">
					                	<?php				
					                        	if ($getuserid!=$r[userid]){
													...
													$follow='<a href="javascript:void()" onclick="follow('.$r[userid].')" class="button yiyaoqingBtn blue small" id="follow'.$r[userid].'">取消邀请</a>';	
													}
					...
					<?
					$userid=$userid."::::::";
					$friend2_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.groupid in(1,2) and b.yaoqing like '%$userid%' and b.userid not in($whe) and b.userid not in($yao) order by a.userid desc";
