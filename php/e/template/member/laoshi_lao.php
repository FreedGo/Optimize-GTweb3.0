...
//申请加入的
$frie_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.groupid=4 and yaoqing like '%$userid%' order by a.userid desc";
...
$friend_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.userid IN ($whe) and a.groupid=4 order by a.userid desc";
...
<!--我的邀请-->
<?
$friend_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.userid IN ($yao) and a.groupid=4 and b.userid not in($whe) order by a.userid desc";
$list=$empire->query($friend_sql);
while($r=$empire->fetch($list))
{	   	
?>
...
<?
$userid=$userid."::::::";
$friend2_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.groupid=4 and b.yaoqing like '%$userid%' and b.userid not in($whe) and b.userid not in($yao) order by a.userid desc";
...
?> 
			...
			<div class="new-code" style="float: left;width: 100%;font: 14px/40px 微软雅黑;padding-left: 10px;">
				...
				<ul class="lh dengdai">
<?
$friend_sql="select * from {$dbtbpre}enewsmember a left join {$dbtbpre}enewsmemberadd b on a.userid=b.userid WHERE a.userid IN ($guanwhe) and a.groupid=4 and b.userid not in($whe) and b.userid not in($yao) and b.userid not in($str) order by a.userid desc";
$list=$empire->query($friend_sql);
while($r=$empire->fetch($list))
{	   	
?>
