...
if($verify_result) {//验证成功
    ...
    if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
		//判断该笔订单是否在商户网站中已经做过处理
			...
			//如果有做过处理，不执行商户的业务程序
$r=$empire->fetch1("select userid,money,shopzhuang from phome_enewsshopdd where ddno=$out_trade_no");
	
	//判断该笔订单是否在商户网站中已经做过处理			

	if($r[shopzhuang]!="已付款"){
$empire->query("UPDATE phome_enewsshopdd SET shopzhuang = '已付款' WHERE ddno =$out_trade_no LIMIT 1");
