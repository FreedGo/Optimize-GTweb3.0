...
(function(){
	var Provider, aFilters = [], nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = '<b class="stream-uploading-ico"></b>' +
					...
					'	<span class="stream-cell-info stream-cell-info1">速度：<em class="stream-speed"></em></span>' +
					'	<span class="stream-cell-info stream-cell-info2">进度：<em class="stream-uploaded"></em></span>' +
					'	<span class="stream-cell-info stream-cell-info3">剩余时间：<em class="stream-remain-time"></em></span>' +
					'</div>',
	...
	Main.prototype = {
		...
		formatSpeed : function(a) {
			// 如果网速低于100KB，则提醒
			var www=Math.round(a / 1024);
				if (www<100) {
					$('.video-upload').prepend('<span class="slow" style="color:red;font-size:14px;position:absolute;left:220px;line-hight:24px;width:200px;">您的网速过慢，请耐心等待</span>');
					$('.slow').remove();
				};
			var b = 0;
