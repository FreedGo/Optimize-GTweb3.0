...
//Feed相关
function follow(id){
	$.post("/e/extend/tuijian/index.php",
		{
			followid:id
		},
		function(data,status){
			 switch(data){
				 case"DelSuccess":jError("取消推荐成功!",{HorizontalPosition : 'center',VerticalPosition:'center',onClosed : function(){
					 $("#follow"+id).text("推荐");
					 $("#follow"+id).removeClass("orange");
					 }});
				 break;
				 case"unknowerror":jError("发生未知错误,请联系管理员!",{HorizontalPosition : 'center',VerticalPosition:'center'});
				 break;
				 case"nofollowme":jError("不能推荐自己哦!",{HorizontalPosition : 'center',VerticalPosition:'center'});
				 break;
				 case"AddSuccess":jSuccess("推荐成功!",{HorizontalPosition : 'center',VerticalPosition:'center',onClosed : function(){
					 $("#follow"+id).text("取消推荐");
					 $("#follow"+id).addClass("orange");
					 }})
				 break;
