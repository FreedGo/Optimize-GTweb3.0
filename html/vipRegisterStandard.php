...
<div class="container" id="qhMsg">
	...
	<form action="" class="form-inline" role="form" method="post">
			...
			<div class="row qinhang-add">
				<div class="col-sm-4">
					<div class="input-group">
						<input type="text" v-model="addresShi" name="addresShi" class="form-control">
		...
		<div class="table-warp customer-statu table-responsive">
			...
			<button type="submit" class="btn btn-primary btn-lg btn-block">确认并提交</button>
		</div>
...
<script>
	/**
	 * 采用VUE进行数据绑定
	 * 数据采集时,把form表单内部的input的name对应数据表字段即可
	 * 数据调用时,php生成数据放置于data内部相应的键值对的键后面即可
	 * @type {Vue}
	 */
	var app = new Vue({
		el: '#qhMsg',
